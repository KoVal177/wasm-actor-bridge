...
mod wasm_impl {
    ...
    /// to [`CallFuture`](crate::CallFuture) and do not appear here.
    ///
    ...
    pub struct EventStream<Evt> {
        receiver:
            futures_channel::mpsc::UnboundedReceiver<Result<WorkerEvent<Evt>, BridgeError>>,
    }

    impl<Evt> EventStream<Evt> {
        pub(crate) fn new(
            receiver: futures_channel::mpsc::UnboundedReceiver<
                Result<WorkerEvent<Evt>, BridgeError>,
            >,
        ) -> Self {
            Self { receiver }
        }
